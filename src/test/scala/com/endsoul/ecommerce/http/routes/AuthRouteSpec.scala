...
class AuthRouteSpec
  ...
  val mockedAuthenticator: Authenticator[IO] = new Authenticator[IO] {
    ...
    override def encodeAccessToken(user: User): IO[String] = IO.pure("accessToken")
    override def encodeRefreshToken(username: String): IO[String] = IO.pure("refreshToken")
  ...
  val mockedAuth: Auth[IO] = new Auth[IO] {
    override def authenticate: jwt.JwtToken => JwtClaim => IO[Option[User]] = _ =>
      _ => IO.pure(Some(Lee))
    override def authenticator: Authenticator[IO] = mockedAuthenticator

    override def signUp(newUserInfo: NewUserInfo): IO[Option[AuthToken]] =
      if (newUserInfo.username === hongUsername)
        IO.pure(Some(AuthToken("accessToken", "refreshToken")))
      else
        IO.pure(None)

    override def login(username: String, password: String): IO[Option[AuthToken]] =
      if (username == leeUsername && password == leePassword)
        IO.pure(Some(AuthToken("accessToken", "refreshToken")))
      else IO.pure(None)

    def refreshAccessToken(token: String): IO[Option[String]] = IO.pure(Some("newAccessToken"))
